...
if __name__ == "__main__":
 ...
 #
                # a = np.fromfile(file_lst, dtype=np.uint16,count=nb_byte_to_read)
                # x1 = np.where(a == 32768) #recherche 0x8000
                # x0 = np.where(a == 32768,a, np.zeros(len(a)))  # recherche 0x8000
                #
                # x2 = np.full(np.shape(x1), 1)
                # x3 = x1-x2 #indice ADC qui a déclanché
                # x4 =x3[:]-1 #test<+
                # x5 = a[x3] #tableau des indices ADC
                #
                # x6 = np.bitwise_and(x5[:],adc2read) #X6 = adc2read si num adc trouvé
                # x7 = np.where(x6[:] != 0, x3[:],np.zeros(len(x6)))
                # x8 = [int(i) for i in x7[0][:] if i]
                # # for i in x7[0][:] :
                # #     print(i)
                # #     if i != 0:
                # #         x8.append(i)
                ...
                # t0 = perf_counter()
                #
                # b1 = np.full(len(x8),fill_value=1)
                # b2 = np.full(len(x8), fill_value=2)
                # b4 = np.full(len(x8), fill_value=4)
                # b8 = np.full(len(x8), fill_value=8)
                # b16 = np.full(len(x8), fill_value=16)
                # b32 = np.full(len(x8), fill_value=32)
                # b64 = np.full(len(x8), fill_value=64)
                # b128 = np.full(len(x8), fill_value=128)
                # b256 = np.full(len(x8), fill_value=256)
                # b512 = np.full(len(x8), fill_value=512)
                # b1024 = np.full(len(x8), fill_value=1024)
                #
                #
                #
                # x9 = np.take(a,x8)
                #
                # r1 =   np.bitwise_and(x9[:],b1[:],out = a[]
                # r2 =   x9&b2
                # r4 =   x9&b4
                # r8 =   x9&b8
                # r16 =  x9&b16
